...
public abstract class GameEngine {
    ...
    // Number of ticks (ie, iterations of the game loop) since the game started
    // running
    protected int ticksElapsed;

    // the graphical window where the game is played and the entities are drawn
    private GUIWindow1 window;
    ...
    // First element is drawn beneath all entities, last element ontop of all
    // entities.
    protected ArrayList<Enemies> displayList;
    ...
    private void initGame(int gameWidth, int gameHeight) {
        displayList = new ArrayList<Enemies>();
        // We need to pass GUIWindow a reference to our displayList at instantation
        window = new GUIWindow1(gameWidth, gameHeight, displayList);
    ...
    private void handlePlayerInput() {
        ...
        // player could be pressing multiple keys simultaneously, ex: moving
        // diagonally).
        ...
        if (keysPressed.contains(LEFT_KEY)) {
            leftTrue = true;
        } else {
            leftTrue = false;
        }
        if (keysPressed.contains(RIGHT_KEY)) {
            rightTrue = true;
        } else {
            rightTrue = false;
        }
    ...
    // GetEntities the splash screen image of the window.
    // returns the filename of the splash screen image, or null if there is no
    // background.
    public String getSplashImage() {
        return window.getSplashImage();
    }

    // GetEntities the current background color of the game window.
    ...
    protected ArrayList<Enemies> checkCollision(int x, int y) {
        ArrayList<Enemies> collisions = new ArrayList<Enemies>();
        for (int i = 0; i < displayList.size(); i++) {
            Enemies e = displayList.get(i);
    ...
    // Checks to see if the argument entity is colliding with ANYTHING in the game
    // window.
    // This method returns an ArrayList containing any Entities that are colliding
    // with
    // the argument Entity
    protected ArrayList<Enemies> checkCollision(Enemies toCheck) {
        ArrayList<Enemies> collisions = new ArrayList<Enemies>();
        for (Enemies e : displayList) {
    ...
    // Sets the debug text to be drawn in the lower-left hand corner of the game
    // window.
    ...
    // a value of 100 indicates the game is running at 100% speed, ie the default
    // game speed.
    // 150 and 50 would indicate game running 50% faster and 50% slower,
    // respectively.
    ...
    public boolean checkOnGround(Player player, Enemies toCheck) {
        ArrayList<Enemies> collisionList = checkCollision(toCheck);
    ...
    protected abstract boolean updateGame();
    ...
    protected abstract MouseEvent handleMouseClick(MouseEvent click);
