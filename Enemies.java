...
public abstract class Enemies {
    ...
    public Enemies(double x, double y, int width, int height, String imageName){
        this.x = x;
        this.y = y;
        this.width = width;
        this.height = height;
    ...
    public boolean isCollidingWith(Enemies other){
        int othersX = (int)other.getX();
        int othersXRange = other.getWidth();
        int othersY = (int)other.getY();
        int othersYRange = other.getHeight();
        //tried to establish a hitbox using the corners of the entity in question
        for (int i=othersX; i<=(int)othersXRange+othersX;i++){
            for (int z = othersY; z<=othersY+othersYRange; z++){
                boolean currentCheck = isCollidingWith(i,z);
